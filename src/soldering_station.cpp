...
//
#ifdef DEBUG_PRINT
void DebugPrint(int mode,  const char* str)
{
  static uint32_t timer1 = 0;
  uint32_t _time = millis();
  uint16_t _temp, t1, t2;
  ...
  if (mode == 0)
  {
    ...
    t1 = analogRead(probePIN);
    delayMicroseconds(50);
    t2 = analogRead(probePIN);
    _temp = t1 + t2 + 1; // average of two measurements
    _temp >>= 1;

    Serial.print("IronTemp: ");
  ...
  {
    counter1++;
    Serial.print("EEPROM write: "); Serial.println(counter1);
  }
  if(mode > 1)
  {  
...
private:
  ...
  const uint16_t def_min = 280; //410;               // Default minimum temperature
  const uint16_t def_max = 606; //700;               // Default maximum temperature
  const uint16_t def_set = 280; //450;               // Default setup temperature
};
...
{
  DebugPrint( 10, "saveTemp()+");
  if (!save_calibration && (t == Config.temp))
    return true;
  Config.temp = t;
  save_calibration = false;
  DebugPrint(2, "saveTemp");
  return save();
}

void CONFIG::saveConfig(byte off, bool cels, byte bright)
{
  DebugPrint( 10, "saveConfig()+");
  if (off > 30)
    off = 0;
  if (bright > 15)
    bright = 15;
...
{
  DebugPrint( 10, "getCalibrationData()+");
...
bool CONFIG::load(void)
{
  ...
  if (is_valid)
  {
    ...
    if ((Config.temp > Config.temp_max) || (Config.temp < Config.temp_min))
      Config.temp = def_set;
    if ((Config.off_timeout > 30))
      Config.off_timeout = 0;
  }
  return is_valid;
}
...
void CONFIG::setDefaults(bool Write)
{ // Restore default values
  DebugPrint( 10, "setDefaults()+");
  Config.temp = def_set;
...
bool BUTTON::buttonTick(void)
{ // When the button pressed for a while, generate periodical ticks

  bool keyUp = digitalRead(buttonPIN); // Read the current state of the button
  uint32_t now_t = millis();
  if (!keyUp && (now_t - pt > shortPress))
  { // The button have been pressed for a while
...
void DSPL::tSet(uint16_t t, bool celsius)
{
  number(t);
  animate_type = 0;
  LedControl::setRow(0, 3 - 0, s_t_dot); //<-(ZAP) LedControl::setRow(0, 0, s_t_dot);
}
...
void DSPL::pSet(byte p)
{
  number(p);
  animate_type = 0;
  LedControl::setChar(0, 3 - 0, 'P', true); //<-(ZAP) LedControl::setChar(0, 0, 'P', true);
}
...
void DSPL::tempLim(byte indx, uint16_t temp)
{
  ...
  if (indx > 0)
    sym = s_low;
  LedControl::setRow(0, 3 - 0, sym); //<-(ZAP) LedControl::setRow(0, 0, sym);
}
...
{
  animate_type = 0;
  LedControl::setRow(0, 3 - 0, s_up); //<-(ZAP) LedControl::setRow(0, 0, s_up);
}

void DSPL::msgLower(void)
{
  animate_type = 0;
  LedControl::setRow(0, 3 - 0, s_low); //<-(ZAP) LedControl::setRow(0, 0, s_low);
}
...
void DSPL::heating(void)
{
  ...
  LedControl::setRow(0, 3 - 0, a_heating[0]); //<-(ZAP) LedControl::setRow(0, 0, a_heating[0]);
}
...
void DSPL::cooling(void)
{
  if (animate_type != 2)
  {
    ...
    LedControl::setRow(0, 3 - 0, a_cooling[0]); //<-(ZAP) LedControl::setRow(0, 0, a_cooling[0]);
  }
}
...
void DSPL::setupMode(byte mode, byte p)
{
  ...
  case 0:
    number(p);
    LedControl::setChar(0, 3 - 0, 'A', false); //<-(ZAP) LedControl::setChar(0, 0, 'A', false);
    LedControl::setRow(0, 3 - 1, s_low);       //<-(ZAP) LedControl::setRow(0, 1, s_low);
    break;
  case 1:
    clear();
    LedControl::setRow(0, 3 - 0, s_U);     //<-(ZAP) LedControl::setRow(0, 0, s_U);
    LedControl::setRow(0, 3 - 1, s_n_dot); //<-(ZAP) LedControl::setRow(0, 1, s_n_dot);
    if (p)
      LedControl::setRow(0, 3 - 3, 0b01001110); // 'C'  //<-(ZAP) LedControl::setRow(0, 3, 0b01001110);
    else
      LedControl::setChar(0, 3 - 3, 'F', false); //<-(ZAP) LedControl::setChar(0, 3, 'F', false);
    break;
  case 2:
    number(p);
    LedControl::setRow(0, 3 - 0, 0b00011111); // 'b' //<-(ZAP) LedControl::setRow(0, 0, 0b00011111);
    LedControl::setRow(0, 3 - 1, 0b10000101); // 'r.' //<-(ZAP) LedControl::setRow(0, 1, 0b10000101);
    break;
  default:
    break;
  }
...
void DSPL::show(void)
{
  if (animate_type)
  {
    ...
    {
      if (++animate_index >= animate_count)
        animate_index = 0;
      switch (animate_type)
      {
      case 1:
        animate_ms = millis() + animate_speed;
        this->setRow(0, 3 - 0, a_heating[animate_index]); //<-(ZAP) this->setRow(0, 0, a_heating[animate_index]);
        break;
      case 2:
        animate_ms = millis() + animate_speed * 5;
        this->setRow(0, 3 - 0, a_cooling[animate_index]); //<-(ZAP) this->setRow(0, 0, a_cooling[animate_index]);
        break;
      default:
        break;
      }
...
void DSPL::number(int data)
{
  ...
  {
    for (byte i = 1; i <= 3; ++i)
      LedControl::setChar(0, 3 - i, '-', false); //<-(ZAP) LedControl::setChar(0, i, '-', false);
    return;
  }
  byte i;
  for (i = 0; i < 4; ++i)
  {
    ...
    byte s = data % 10;
    data /= 10;
    LedControl::setDigit(0, 3 - (3 - i), s, false); //<-(ZAP) LedControl::setDigit(0, 3-i, s, false);
  }
  for (; i < 4; ++i)
  {
    LedControl::setChar(0, 3 - (3 - i), ' ', false); //<-(ZAP) LedControl::setChar(0, 3-i, ' ', false);
  }
}
...
{
  for (byte i = 0; i < 4; ++i)
    LedControl::setRow(0, 3 - i, msg[i]); //<-(ZAP) LedControl::setRow(0, i, msg[i]);
  animate_type = 0;
}
...
{
  Serial.print(t_set - t_curr);
  Serial.print(": ");
  Serial.print("[ ");
  Serial.print(temp_hist[0]);
  Serial.print(", ");
  Serial.print(temp_hist[1]);
  Serial.print(", ");
  Serial.print(t_curr);
  Serial.print(" ] kp = ");
  Serial.print(kp);
...
private:
  ...
  HISTORY h_power;
  HISTORY h_temp;
  const uint16_t temp_cold = 70;      //255;          // The cold temperature to touch the iron safely
  const uint16_t temp_no_iron = 1000; //720;          // Sensor reading when the iron disconnected
...
{
  if (!unit_celsius)
    return map(temp, temp_min, temp_max, temp_minF, temp_maxF);
...
{
  if (Power == 0)
  { // To switch off the iron, set the power to 0
    fix_power = false;
    actual_power = 0;
    digitalWrite(hPIN, LOW);
    return true;
...
void mainSCREEN::init(void)
{
  ...
  cool_notified = !used;
  if (used)
  { // the iron was used, we should save new data in EEPROM
...
{
  if ((!force_redraw) && (millis() < update_screen))
    return;
...
void tuneSCREEN::show(void)
{
  ...
  update_screen = millis() + 1000;
  if (mode != 0)
  { // Selected upper or lower temperature
...
{
  if (mode == 0)
  { // No limit is selected, list the menu
    switch (value)
    {
    case 2:
      pD->msgDefault();
...
{ // The rotary button pressed
  if (mode == 0)
  { // select upper or lower temperature limit
    int val = pEnc->read();
    if (val == 2)
    {                          // load defaults
      pCfg->setDefaults(true); // Write default config to the EEPROM
      if (main)
        return main; // Return to the main screen
    }
    if (val == 3)
    {
    ...
    {
      if (nextL)
        return nextL; // Return to the previous menu
    }
    mode = val + 1;
    pD->clear();
    pD->msgTune();
  ...
  {                               // upper or lower temperature limit just setup
...
SCREEN *tuneSCREEN::menu_long(void)
{
  ...
  if (all_data)
  { // save calibration data. Config will be written to the EEPROM later on
...
void loop()
{
  ...
  if ((pCurrentScreen == &wrkScr) && !iron_on)
  ...
  { // return to the main screen by timeout
    pCurrentScreen = nxt;
    pCurrentScreen->init();
  }
